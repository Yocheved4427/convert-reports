"""
Type-B/N transformer – backward-compatible ``BaseTransformer`` wrapper.

``TypeBTransformer`` (aliased as ``TypeNTransformer``) implements the legacy
``BaseTransformer`` interface used by ``ReportProcessorFactory``.  All
transformation logic lives in ``TypeBTransformationStrategy``; this class is a
thin delegation shell.
...
class TypeBTransformer(BaseTransformer):
    """Backward-compatible ``BaseTransformer`` for Type-B reports.

    Delegates to ``ValidatingStrategyDecorator(TypeBTransformationStrategy())``
    ...
    def transform(
        ...
        """Transform *report* using the canonical Type-B strategy.
        ...
        from src.strategies.type_b_strategy import TypeBTransformationStrategy  # lazy
        from src.strategies.validating_strategy_decorator import ValidatingStrategyDecorator  # lazy
        service = TransformationService(
            strategy_registry={
                ReportType.TYPE_B.value: ValidatingStrategyDecorator(
